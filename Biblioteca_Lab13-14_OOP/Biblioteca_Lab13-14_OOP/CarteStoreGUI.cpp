...
void CarteStoreGUI::initializeGUIComponents() {
	...
	//criteriilor de sortare pe care le avem (dupa an+gen 
	//si titlu) + butonul de sortare
	...
	int noLines = 10;
	int noColumns = 4;
	this->tableBooks = new QTableWidget{ noLines, noColumns };
...
void CarteStoreGUI::reloadCarteList(vector<Carte> books) {
	this->tableBooks->clearContents();
	this->tableBooks->setRowCount(int(books.size()));
...
void CarteStoreGUI::connectSignalsSlots() {
	...
	QObject::connect(btnSortBooks, &QPushButton::clicked, [&]() {
		if (this->radioSrtAnGen->isChecked())
			this->reloadCarteList(srv.sortByGenAn());
		else if (this->radioSrtTitlu->isChecked())
			this->reloadCarteList(srv.sortByTitlu());
		else if (this->radioSrtAutor->isChecked())
			this->reloadCarteList(srv.sortByAutor());
		});
	...
	QObject::connect(btnFilterBooks2, &QPushButton::clicked, [&]() {
		...
		this->reloadCarteList(srv.filtrareAnMaiMic(filterC));
		});

	QObject::connect(btnCauta, &QPushButton::clicked, this, &CarteStoreGUI::guiWindowCauta);
	QObject::connect(btnModifica, &QPushButton::clicked, this, &CarteStoreGUI::guiWindowModifica);
...
void CarteStoreGUI::reloadBasketList2() {
...
void CarteStoreGUI::guiWindowCauta()
{
...
void CarteStoreGUI::guiWindowModifica()
{
...
void CarteStoreGUI::guiAddButtonsDinamic(string newgen)
{
	int gasit = 0;
...
void CarteStoreGUI::guiDeleteButtonsDinamic(string gen)
{
	int cont = 0;
...
void CarteStoreGUI::reloadBasketList(vector<Carte> books) {
...
void CarteStoreGUI::guiInitializareButoane()
{
...
void CarteStoreGUI::guiAddCarteBasket() {
	try {
		//preluare detalii din QLineEdit-uri
...
void CarteStoreGUI::createHorizontalGroupBox()
{
